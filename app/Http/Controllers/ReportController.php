...
class ReportController extends Controller
{
    ...
    {
        $request->validate([
            'view_type' => 'nullable|in:general,daily',
    ...
    {
        $groupedMovements = $this->fetchGeneralMovements($request);
    ...
    private function dailyReport(Request $request, $clients)
    {
        ...
        [$installationsFormatted, $removalsFormatted] = $this->fetchDailyMovements($date, $clientId);
    ...
    /**
     * Devuelve los movimientos para la vista general aplicando los filtros del request.
     */
    private function fetchGeneralMovements(Request $request)
    {
        ...
        return $this->groupAndTransformMovements($movements);
    }

    /**
     * Devuelve [instalaciones, retiros] del día indicado, filtrados opcionalmente por cliente.
     */
    private function fetchDailyMovements(string $date, ?string $clientId): array
    {
        $clientFilter = function ($query) use ($clientId) {
            if (! $clientId) {
                return;
            }
        ...
        return [
            $this->formatDailyMovements($installations),
            $this->formatDailyMovements($removals),
        ];
    }

    /**
     * Genera y descarga el PDF del informe (general o diario) según los filtros recibidos.
     */
    public function pdf(Request $request)
    {
        ...
        if ($viewType === 'daily') {
            $date = $request->get('daily_date', now()->format('Y-m-d'));
            [$installations, $removals] = $this->fetchDailyMovements($date, $request->client_id);
        ...
        $movements = $this->fetchGeneralMovements($request);
        ...
        return $pdf->download('reporte-general-'.now()->format('Y-m-d_H-i').'.pdf');
    }
